...
public class ProfileServiceImpl implements ProfileService {
    ...
    public List<ProfileResponse> getAllProfiles() {
        ...
        List<RoleRepresentation> allRoles = identityClient.getAllRoles("Bearer " + token, idpProperties.getRealm());

        return profileRepository.findAll().stream()
                .map(profile -> mapProfileWithRoles(profile, allRoles, token))
                .collect(Collectors.toList()); // Chuyển đổi danh sách profile sang profile response và kèm theo vai trò
    }

    // Map một profile với các vai trò của nó
    private ProfileResponse mapProfileWithRoles(Profile profile, List<RoleRepresentation> allRoles, String token) {
        List<String> userRoles;
        ProfileResponse response = profileMapper.toProfileResponse(profile); // Chuyển entity sang DTO

        try {
            userRoles = identityClient
            ...
            List<RoleResponse> roles = allRoles.stream()
                    .filter(role -> userRoles.contains(role.getName())) // Chỉ lấy những vai trò người dùng có
    ...
    public ProfileResponse getMyProfile() {
        String userId = SecurityContextHolder.getContext().getAuthentication().getName(); // Lấy userId từ token
        var profile = findProfileByUserId(userId);
        return mapProfileWithRoles(profile, identityClient.getAllRoles("Bearer " + getToken(), idpProperties.getRealm()), getToken());
    }
    ...
    public ProfileResponse registerProfile(RegistrationRequest request) {
        try {
            String token = getToken();
            String userId = extractUserId(identityClient.createUser(
            ...
            return mapProfileWithRoles(profile, identityClient.getAllRoles("Bearer " + token, idpProperties.getRealm()), token);
        } catch (FeignException e) {
    ...
    private void updateBasicPersonalInfo(String userId, String firstName, String lastName) {
        ...
        if (firstName != null || lastName != null) {
            var updateParam = UserUpdateParam.builder()
                    .firstName(firstName)
                    .lastName(lastName)
                    .build();
            identityClient.updateUser("Bearer " + getToken(), idpProperties.getRealm(), userId, updateParam);
        }
    }
    ...
    public void updateProfileByAdmin(String userId, ProfileUpdateRequest request) {
        var profile = findProfileByUserId(userId);
    ...
    private Profile findProfileByUserId(String userId) {
        return profileRepository.findByUserId(userId).orElseThrow(() -> new AppException(ErrorCode.USER_NOT_EXISTED));
    }
    ...
    private void updatePasswordInKeycloak(String userId, String newPassword) {
        ...
        identityClient.updatePassword("Bearer " + getToken(), idpProperties.getRealm(), userId, credential);
    }
    ...
    private void assignRoleToUserInKeycloak(String userId, String roleId, String roleName) {
        ...
        try {
            identityClient.assignUserRoles("Bearer " + getToken(), idpProperties.getRealm(), userId, List.of(role));
    ...
    private void changeUserStatus(String userId, boolean enabled, Banned bannedStatus) {
        ...
        identityClient.updateUserStatus("Bearer " + getToken(), idpProperties.getRealm(), userId, userUpdate);
